...
FirebaseLockNotificationsRemoteDataSource injectFirebaseLockNotificationsRemoteDataSource() {
  return FirebaseLockNotificationsRemoteDataSourceImpl(database: injectFirebaseLockNotificationsDatabase());
}
...
class FirebaseLockNotificationsRemoteDataSourceImpl
  ...
  Future<List<Notification>> getNotificationsList({required String lockId}) async{
    try {
      var response = await database.getNotificationsList(lockId: lockId).timeout(const Duration(seconds: 60));
