...
FirebaseContactsRemoteDataSource injectFirebaseContactsRemoteDataSource(){
  return FirebaseContactsRemoteDataSourceImpl(database: injectFirebaseContactsDatabase());
}

class FirebaseContactsRemoteDataSourceImpl implements FirebaseContactsRemoteDataSource {
  ...
  Future<bool> contactExist({required String firstUserUID, required String secondUserUID}) async{
    try{
      var response = await database.contactExist(firstUserUID: firstUserUID, secondUserUID: secondUserUID);
      if(response){
  ...
  Future<Contact> createNewContact({required ContactDTO contact})async {
    try{
      var response = await database.createContact(contact: contact);
  ...
  Future<List<ContactDTO>> getFirstUserContact({required String uid}) async{
    try{
      var response = await database.getFirstUserContact(uid: uid);
      return response;
    } on FirebaseException catch (e) { // handle firebase exception in en of ar
      throw FirebaseDatabaseException(errorMessage: e.code);
    }on IOException {
  ...
  Future<List<ContactDTO>> getSecondUserContact({required String uid}) async{
    try{
      var response = await database.getSecondUserContact(uid: uid);
      return response;
    } on FirebaseException catch (e) { // handle firebase exception in en of ar
      throw FirebaseDatabaseException(errorMessage: e.code);
    }on IOException {
